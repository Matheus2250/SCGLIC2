...
interface WidgetConfig {
  ...
  title: string;
  type: ChartType;
  xField: keyof Qualificacao | string;
  yField?: keyof Qualificacao | string; // opcional (quebrar por)
  metric: 'count' | 'value'; // value = soma de valor_estimado
...
const fieldLabels: { label: string; key: keyof Qualificacao | string }[] = [
  { label: 'Área Demandante', key: 'area_demandante' },
  { label: 'Responsável pela Instrução', key: 'responsavel_instrucao' },
  { label: 'Modalidade', key: 'modalidade' },
  { label: 'Status', key: 'status' },
  { label: 'Ano', key: 'ano' },
  { label: 'Palavra-chave', key: 'palavra_chave' },
  { label: 'Total de Qualificações', key: '__total__' },
];
...
const aggregate = (items: Qualificacao[], xField: string, metric: 'count'|'value', yField?: string): SeriesResult => {
  const safe = (v: any) => (v === null || v === undefined || v === '') ? '—' : v;
  const valueOf = (it: any) => Number(it?.valor_estimado || 0);
  if (xField === '__total__') {
    if (!yField) {
...
const DashboardBuilderQualificacao: React.FC<Props> = ({ storageKey }) => {
  ...
  useEffect(() => {
    const fetchAll = async () => {
      try {
        const limit = 500; let skip = 0; let all: Qualificacao[] = [];
        for (let i = 0; i < 20; i++) {
          const batch = await qualificacaoService.getAll(skip, limit);
  ...
  return (
    <Box>
      <Box sx={{ display:'flex', justifyContent:'space-between', alignItems:'center', mb:2 }}>
        <Typography variant="h6">Seu painel (Qualificação)</Typography>
            ...
            <Box sx={{ display:'grid', gap:2, mt:1 }}>
              ...
              <FormControl fullWidth>
                ...
                <Select label="Métrica" value={editing.metric} onChange={(e) => setEditing({ ...editing, metric: e.target.value as any })}>
                  ...
                  <MenuItem value="value">Valor Estimado</MenuItem>
                </Select>
              </FormControl>
              <FormControl fullWidth>
...
export default DashboardBuilderQualificacao;
